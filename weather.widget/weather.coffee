...
appearance =
  ...
  color         : '#444'            # configure your colors here
  darkerColor   : '#222'
  baseFontSize  : 38                # the base font size, the size of the temperature
...
command: "$(ps -o comm= $PPID | sed -e 's/UM-LM\^H/Ü/') .weather.widget/weather-api \
                            \"#{options.city}\" \
                            \"#{options.region}\" \
                            #{options.units} \
